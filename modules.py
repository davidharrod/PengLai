...
def dequantize_verts(verts, n_bits, add_noise=False):
    ...
    if add_noise:
        verts += tf.random_uniform(
            tf.shape(verts)) * (1 / float(range_quantize))
    return verts
    ...
    if not forget:
        ...
        def grad_fn(x, wqkv, wo, attention_bias, norm_scale, norm_bias, dy):
            ...
            with tf.control_dependencies([dy]):
                n = common_layers.layer_norm_compute(x, epsilon, norm_scale,
                                                     norm_bias)
                wqkv_split = tf.unstack(wqkv, num=num_heads)
                wo_split = tf.unstack(wo, num=num_heads)
                deps = []
        ...
        for layer_num in range(self.num_layers):
            with tf.variable_scope('layer_{}'.format(layer_num)):
                ...
                if self.memory_efficient:
                    ...
                    if self.layer_norm:
                        res = common_layers.layer_norm(res,
                                                       name='self_attention')
                    res = common_attention.multihead_attention(
                        ...
                        name='self_attention')
                if self.re_zero:
                    res *= tf.get_variable('self_attention/alpha',
                                           initializer=0.)
                if dropout_rate:
            ...
            with tf.variable_scope('layer_{}'.format(layer_num)):
                ...
                if sequential_context_embeddings is not None:
                    ...
                    res = common_attention.multihead_attention(
                        ...
                        name='cross_attention')
                    if self.re_zero:
                        res *= tf.get_variable('cross_attention/alpha',
                                               initializer=0.)
                    if dropout_rate:
                        res = tf.nn.dropout(res, rate=dropout_rate)
                    x += res
...
class VertexModel(snt.AbstractModule):
  ...
  """
    ...
    def _embed_class_label(self, labels):
        ...
        return snt.Embed(vocab_size=self.num_classes,
    ...
    def _prepare_context(self, context, is_training=False):
        ...
        if self.class_conditional:
            global_context_embedding = self._embed_class_label(
                context['class_label'])
        else:
    ...
    def _embed_inputs(self, vertices, global_context_embedding=None):
        ...
        # Continuous vertex value embeddings
            ...
            vert_embeddings = tf.layers.dense(dequantize_verts(
        ...
        if global_context_embedding is None:
            zero_embed = tf.get_variable('embed_zero',
        ...
        embeddings = vert_embeddings + (coord_embeddings +
                                        pos_embeddings)[None]
    ...
    """
        ...
        def _stopping_cond(i, samples, cache):
            ...
            return tf.reduce_any(
                tf.reduce_all(tf.not_equal(samples, 0), axis=-1))
        ...
        cache, cache_shape_invariants = self.decoder.create_init_cache(
            num_samples)
        _, v, _ = tf.while_loop(cond=_stopping_cond,
                                body=_loop_body,
        ...
        # first nonzero index.
        stop_index_completed = tf.argmax(tf.cast(tf.equal(v, 0), tf.int32),
                                         axis=-1,
                                         output_type=tf.int32)
        # For incomplete samples the stopping index is just the maximum index.
        stop_index_incomplete = (max_sample_length * 3 *
                                 tf.ones_like(stop_index_completed))
        stop_index = tf.where(completed, stop_index_completed,
        ...
        vertices_mask = tf.cast(
            tf.range(max_sample_length)[None] < num_vertices[:, None],
            tf.float32)

        if recenter_verts:
            vert_max = tf.reduce_max(vertices - 1e10 *
                                     (1. - vertices_mask)[..., None],
                                     axis=1,
                                     keepdims=True)
            vert_min = tf.reduce_min(vertices + 1e10 *
                                     (1. - vertices_mask)[..., None],
                                     axis=1,
                                     keepdims=True)
...
class ImageToVertexModel(VertexModel):
  ...
  """
    ...
    Args:
      ...
      name: Name of variable scope
    """
        super(ImageToVertexModel,
              self).__init__(decoder_config=decoder_config,
    ...
    def _prepare_context(self, context, is_training=False):
        ...
        image_coord_embeddings = tf.layers.dense(image_coords,
                                                 self.embedding_dim,
                                                 use_bias=True,
...
class VoxelToVertexModel(VertexModel):
  ...
  """
    ...
    def _prepare_context(self, context, is_training=False):
        ...
        voxel_embeddings = self.res_net(voxel_embeddings,
                                        is_training=is_training)
        ...
        voxel_coord_embeddings = tf.layers.dense(voxel_coords,
                                                 self.embedding_dim,
                                                 use_bias=True,
  ...
  """
    ...
    def _prepare_context(self, context, is_training=False):
        ...
        if self.class_conditional:
            ...
            global_context_embedding = None
        vertex_embeddings = self._embed_vertices(context['vertices'],
    ...
    def _embed_vertices(self, vertices, vertices_mask, is_training=False):
        ...
        if self.use_discrete_vertex_embeddings:
            ...
            vertex_embeddings = tf.layers.dense(vertices,
                                                self.embedding_dim,
                                                use_bias=True,
        ...
        # tokens
        stopping_embeddings = tf.get_variable('stopping_embeddings',
                                              shape=[1, 2, self.embedding_dim])
        stopping_embeddings = tf.tile(stopping_embeddings,
                                      [tf.shape(vertices)[0], 1, 1])
        vertex_embeddings = tf.concat([stopping_embeddings, vertex_embeddings],
                                      axis=1)
    ...
    def _embed_inputs(self,
        ...
        face_embeddings = tf.gather(vertex_embeddings,
                                    faces_long,
                                    batch_dims=1)
    ...
    def _create_dist(self,
        ...
        # Embed inputs
        decoder_inputs = self._embed_inputs(faces_long, vertex_embeddings,
        ...
        f_verts_mask = tf.pad(vertices_mask, [[0, 0], [2, 0]],
                              constant_values=1.)[:, None]
        logits *= f_verts_mask
    ...
    """
        ...
        cache, cache_shape_invariants = self.decoder.create_init_cache(
            num_samples)
        _, f, _ = tf.while_loop(cond=_stopping_cond,
                                body=_loop_body,
                                loop_vars=(0, samples, cache),
                                shape_invariants=(tf.TensorShape([]),
                                                  tf.TensorShape([None, None]),
        ...
        max_one_ind = tf.reduce_max(tf.range(sample_length)[None] *
                                    tf.cast(tf.equal(f, 1), tf.int32),
                                    axis=-1)
        zero_inds = tf.cast(
            tf.argmax(tf.cast(tf.equal(f, 0), tf.int32), axis=-1), tf.int32)
        num_face_indices = tf.where(complete_samples, zero_inds,
                                    max_one_ind) + 1

        # Mask faces beyond stopping token with zeros
        # This mask has a -1 in order to replace the last new face token with zero
        faces_mask = tf.cast(
            tf.range(sample_length)[None] < num_face_indices[:, None] - 1,
            tf.int32)
        f *= faces_mask
        # This is the real mask
        faces_mask = tf.cast(
            tf.range(sample_length)[None] < num_face_indices[:, None],
            tf.int32)
        ...
        if only_return_complete:
            ...
            num_face_indices = tf.boolean_mask(num_face_indices,
                                               complete_samples)
            context = tf.nest.map_structure(
                lambda x: tf.boolean_mask(x, complete_samples), context)
            complete_samples = tf.boolean_mask(complete_samples,
                                               complete_samples)
