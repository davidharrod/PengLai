...
def random_shift(vertices, shift_factor=0.25):
    ...
    shift = tfd.TruncatedNormal(tf.zeros([1, 3]), shift_factor * 255,
...
def make_vertex_model_dataset(ds, apply_random_shift=False):
    ...
    def _vertex_model_map_fn(example):
        ...
        example['vertices_flat_mask'] = tf.ones_like(example['vertices_flat'],
                                                     dtype=tf.float32)
        return example
...
def make_face_model_dataset(ds,
...
def quantize_verts(verts, n_bits=8):
    ...
    max_range = 0.5
    range_quantize = 2**n_bits - 1
    verts_quantize = (verts - min_range) * range_quantize / (max_range -
                                                             min_range)
...
def plot_meshes(mesh_list,
    ...
    for p_inc, mesh in enumerate(mesh_list):
        ...
        for key in [
                'vertices', 'faces', 'vertices_conditional', 'pointcloud',
                'class_name'
        ]:
        ...
        if mesh['vertices_conditional'] is not None:
            ax.scatter3D(mesh['vertices_conditional'][:, 0],
        ...
        if mesh['pointcloud'] is not None:
            ax.scatter3D(mesh['pointcloud'][:, 0],
                         mesh['pointcloud'][:, 1],
                         mesh['pointcloud'][:, 2],
                         lw=0.,
                         s=2.5 * vert_size,
